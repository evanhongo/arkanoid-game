...
export default function App() {
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      switch (e.key) {
        case "ArrowLeft":
          updateState((draft) => {
            draft.paddleX = draft.paddleX - 15 < 0 ? 0 : draft.paddleX - 15;
          });
          break;
        case "ArrowRight":
          updateState((draft) => {
            draft.paddleX =
              draft.paddleX + 15 > 1080 ? 1080 : draft.paddleX + 15;
          });
          break;
        default:
      }
  ...
  const [start, stop] = useInterval(() => {
    updateState((draft) => {
      ...
      draft.ball.y =
        tmpX >= draft.paddleX &&
        tmpX <= draft.paddleX + PADDLE_WIDTH &&
        tmpY + BALL_HEIGHT >= PADDLE_Y
  ...
  useEffect(() => {
    updateState((draft) => {
      ...
      // hit the wall
        ...
        draft.ball.dx =
          state.ball.x + BALL_WIDTH === CANVAS_WIDTH || state.ball.x === 0
            ? -state.ball.dx
            : state.ball.dx;

      draft.ball.dy =
        state.ball.y === 0 ||
        (state.ball.y === PADDLE_Y - BALL_HEIGHT &&
          state.ball.x >= state.paddleX &&
          state.ball.x <= state.paddleX + PADDLE_WIDTH)
          ? -state.ball.dy
          : state.ball.dy;
  ...
  useEffect(() => {
    if (state.ball.y >= CANVAS_HEIGHT) stop();
  }, [state.ball.y]);
  ...
  return (
    <>
      ...
      <img id="paddle" src={paddleImg} style={{ display: "none" }} />
